...
Importing a model in A-Frame is just one line. It places the mech in the center so we'll move it back and rotate it a little. Download the [mech.gltf](https://www.webgamedev.com/blog/mech/mech.gltf) file and replace the content of `a-scene` with:
...
Let's make the mech move by having it walking a circle in A-Frame. This means that we need to:
...
Having it walk in a circle requires adding a second enitity that offsets the model and defines the animation. After some tweaking the code in the scene entity looks like this:
...
<video autoPlay loop muted style={{ margin: 'auto', maxWidth: 500, borderRadius: 6 }}>
...
There's tons of parameters that can be tweaked, read about them in the [documentation](https://github.com/supermedium/aframe-environment-component#parameters). Here are some screenshots for `default`, `tron` and `yavapai` (shadows will be added in the next section):
...
<video autoPlay loop muted style={{ margin: '24px auto 16px', maxWidth: 500, borderRadius: 6 }}>
