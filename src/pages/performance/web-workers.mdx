...
By default, data sent via `postMessage` is **copied** (structured clone), which is fine for small payloads but expensive for large arrays. [Transferable objects](https://developer.mozilla.org/en-US/docs/Web/API/Web_Workers_API/Transferable_objects) (like `ArrayBuffer`) can be transferred to the worker with zero copy cost — but the main thread loses access to that data.

## Shared Workers

A [Shared Worker](https://developer.mozilla.org/en-US/docs/Web/API/SharedWorker) can be accessed by multiple browser tabs or iframes from the same origin. This is useful for sharing a single connection (e.g., a WebSocket) across multiple tabs of the same game.
