...
<Callout style={{ marginTop: 30, textAlign: 'center' }}>
  💡 [**WebGamer.io**](https://webgamer.io/) is an example of installable Progressive Web App.
</Callout>
...
- ✅ Chromium-based desktop browsers (Chrome, Edge, Brave...).
- ✅ Chromium-based Android browsers (Chrome, Edge, Brave...).
- ✅ Firefox for Android.
- 😕 Firefox desktop [via an extension](https://addons.mozilla.org/en-US/firefox/addon/pwas-for-firefox/).
- 😕 Safari on iOS and iPadOS, but with **no install prompt** support. Users have to click on **Share**, then **Add to home screen**, which is a terrible process that you will have to explain to your users (see example below).
- ❌ Browsers other than Safari **cannot install PWAs at all** on iOS and iPadOS (Note: [this is changing!](https://webkit.org/blog/13878/web-push-for-web-apps-on-ios-and-ipados/) 🎉).
- ❌ Safari and Opera on desktop.
...
<div style={{ margin: '24px auto', textAlign: 'center' }}>(the red highlights are part of the instructions modal)</div>
...
Offline support is **no longer required** to install a PWA 🎉. If you want your game to work offline, you can use service workers. Service workers can get quite complicated and I would recommend leveraging existing libraries instead of doing it yourself. You can use Google's [Workbox](https://developer.chrome.com/docs/workbox/) to help, but an even easier way to get started is to use a library for your bundler:
