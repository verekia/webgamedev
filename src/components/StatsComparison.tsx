  ...
  return (
    <>
      ...
      <p style={{ marginTop: '0.25rem' }}>
        ...
        />{' '}
        <A style={{ cursor: 'pointer', color: '#777' }} onClick={() => setIsDisclaimerExpanded(!isDisclaimerExpanded)}>
          About these numbers and colors
        </A>
      </p>
      {isDisclaimerExpanded && (
        <p style={{ marginTop: '0.5rem', fontSize: 14, color: '#777' }}>
          Numbers and colors are here to provide an easily-glanceable overview of the activity and traction of each
          tool. While I think community matters for support, it should not be the only decision factor, so pick whatever
          tool works best for your use case.
        </p>
      )}
    </>
  )
